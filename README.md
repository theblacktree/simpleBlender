这是一个模仿blender写的项目，致力于实现blender中的一些功能，目前做了一些功能，后续会继续添加。
...
assimp模型的导入，点光源照射，使用bli-phong

![AssimpModel](images/AssimpModel.png)
...
Execution.rar解压可以直接运行尝试，但是部分功能没有切换功能

双击打开simpleBlenderPBR.exe，左下角是功能区，创建物体、灯光、摄像机，点击创建后会在右上角的管理区显示，右下角是属性区，要使用属性区，必须先用鼠标选中管理区某一个物体/灯光/摄像机，当选中物体时，可以对属性区的变换/材质/物体贴图页面的属性进行设置，当选中灯光时可以对属性区灯光页面进行操作，当选中摄像机时，首先需要先点击左下角功能区的切换摄像机按钮，将当前摄像机设置为选中的摄像机，接着可以对属性区摄像机页面操作，可同步在左侧渲染显示区显示。

右下角的属性区可以用鼠标按中属性值进行左右滑动，实时改变值并显示在显示区。

选中管理区的物体/灯光/摄像机，按delete键删除。

点击左下角加载模型，选中backpack文件夹下的obj文件，载入吉他背包模型。

选中物体在贴图页面添加物体贴图，文件夹内有几张图片可以设置参考。

在贴图页面有一个单独的功能-HDR环境贴图，文件夹内有一个hdr文件，可以载入，上传后为立方体贴图的天空盒。

所有渲染效果都可以通过添加摄像机-选中管理区摄像机-切换摄像机-设置属性区的摄像机属性来获得不同的观察视角。

simpleBlenderAssimpPhong.exe只做了模型加载部分，使用bli-phong渲染模型

# 七. 更新日志

2025.6.5  由于发现摄像机的属性设置不正确，查找原因是因为和物体的位置共用一个lineEdit导致的，现在在camera页增加了设置位置的窗体。

2025.6.17 添加了hdr的导入，使用hdr的球形贴图制作了一个环境，但是对于IBL来说，还只是开始。

增添bli-phong渲染，添加了该着色器文件。

添加了Assimp库导入模型的能力，同时支持部分贴图的pbr渲染和多种2D贴图的bli-phong渲染
